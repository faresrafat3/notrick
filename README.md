...
> Signpost only. Law = ruled boot chain **CONTEXT.md → AGENTS.md → FOUNDATIONAL-BRIEF.md → task-named files** (Owner ruling 2026-09-06 — CONTEXT.md:5-6). Clash: legal file wins.

## The project in one line
An AI mind grown bottom-up — MICs + four charter agencies — coordinating through documents/results on disk, no live orchestrator; Owner = embodied constitution. (pre-pr-024-A CONTEXT head in git history 449aede+; cite by date+entry head, pr-024-A)
...
| `RUN/` · `research/harness-run/` | Rounds-and-receipts machine (R0..R14, SERIES-COMPLETE) · guest research fellow (complete) | Lane writers only (do not edit) |
| `bore/` · `hept/` | The hole lab (COMPLETE — frozen, bore/CORE.md:1) · the honest-record lab (core complete) | Lane writers only |
| `mill/` · `well/` · `phantom/` · `protocols/` | Doctrine mill · reason-law mill · second reader · proposed protocols — dormant artifact lanes (LANES.md) | Read, never silently take over (LANES.md:6) |
| `LANES.md` · `OWNER-DESK.md` | Lane pulse board · the Owner's single decision surface | Append/strike per own design |
| `tools/dsh/` · `tools/audit.sh` · `tools/citecheck.sh` | Hooks publish source (not doctrine, R11/R12) · health-card generator · citation gate | Publish source only · generators (R14) |
...
- `research/ergonomics-tower.md` — measured ergonomics: tower · generator · pr-023 package (2026-09-11, executed same day).
- `research/ergonomics-deep.md` — operating cycle + GENERATE/ROUTE/RECORD invariant + F-1..F-7 (2026-09-11).
- `proposals/pr-024-ergonomics-deep-package.md` — four one-word parts: CONTEXT head-slim · LANES register · citecheck · runbook v0.2 — **ADOPTED A C D 2026-09-19** (B rejected; ruling in proposal + CONTEXT 2026-09-19).

## Task routing (I want to …)
| Intent | Go to |
|---|---|
| see the current state | `bash tools/audit.sh` — the generated card first (ONBOARDING v2) |
| propose something (the guest write-path) | `templates/PROPOSAL.md` → `proposals/` |
| verify integrity / citations | the `INTEGRITY.md` verify command · `bash tools/citecheck.sh` |
| see what waits on the Owner | `OWNER-DESK.md` |
| read the law | `RULES.md` + `charters/` |
| run or audit zero day | `ZERO-DAY-RUNBOOK.md` (pre-flight) + `ZERO-READINESS.md` |
| coordinate with a lane | `LANES.md` (pulse board) — artifact lanes: read, never take over |
| file or read violations | `violations.md` (append-only) |
| understand the system design | `research/ergonomics-tower.md` + `research/ergonomics-deep.md` |

## If you are a new agent
0. `bash tools/audit.sh` first (ONBOARDING v2:7-10) — generated boot card, witness of a moment: quote it, never hand-edit, re-run every boot.
